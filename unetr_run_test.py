...
# 兩種 data_dict，50 病人完整版 & 小量測試版
JSON_50 = "AICUP_training_50.json"
JSON_SMALL = "AICUP_training_smal.json"  # 小量資料（請確認檔案存在）
...
def build_common_args(args, for_test: bool = False):
    ...
    l = [
        ...
        *[str(x) for x in args.depths],
        ...
        str(args.roi_x),
        "--roi_y",
        str(args.roi_y_test if for_test else args.roi_y),
        "--roi_z",
        str(args.roi_z_test if for_test else args.roi_z),
        "--optim",
        args.optim,
        "--lr",
        str(args.lr),
...
def main():
    ...
    parser.add_argument("--data_dict_file_name", default=JSON_50)

    # 🔸 快速小資料測試模式
    parser.add_argument(
        "--quick_debug",
        action="store_true",
        help="使用小量資料與較少 epoch，快速比較不同 UNETR 設定。",
    )
    ...
    # ✅ UNETR：patch_size 要整除 img_size，每維用 16
    ...
    parser.add_argument("--roi_z_test", type=int, default=112)
    ...
    # flags
    parser.add_argument("--pin_memory", action="store_true", default=False)  # 預設關掉，避免 OOM
    ...
    # ====== Quick debug 模式覆蓋一些設定 ======
    if args.quick_debug:
        print("[INFO] 使用 quick_debug 模式：小量資料 + 少 epoch 測試超參。")
        args.data_dict_file_name = JSON_SMALL
        # 少一點 epoch，比較快看到 trend
        args.max_epoch = 40
        args.val_every = 2
        args.max_early_stop_count = 5
        # exp_name 加上後綴，避免跟正式實驗混在一起
    ...
    for i, cfg in enumerate(candidate_configs):
        ...
        if args.quick_debug:
            exp_suffix += "_debug"
        args.exp_name = f"AICUP_unetr_{exp_suffix}"
        ...
        # 資料集與 data_dicts 路徑（同一份 json split，只在 quick_debug 換成 SMALL）
        if args.data_dir is None:
            args.data_dir = args.workspace_dir / "dataset" / args.data_name
        # 注意：這裡要用最新的 args.data_dict_file_name
        ...
        # ====== (2) 訓練完後，立刻對「同一組模型」跑 test ======
        print(f"[INFO] 開始對 {args.exp_name} 跑測試資料 (tune_mode=test)...")

        test_cmd = [
            sys.executable,
            str(tune_py),
            "--tune_mode=test",
        ] + build_common_args(args, for_test=True)

        run(test_cmd)

        print(f"[INFO] {args.exp_name} 測試完成，請到 {args.eval_dir} 查看輸出（例如 CSV / log）。")
