# CardiacSegV2/runners/inferer.py
import os
import time
import importlib
...
def run_infering(
    ...
    unique, counts = np.unique(data['pred'].cpu().numpy(), return_counts=True)
    print("[DEBUG] Pred unique labels & counts:", dict(zip(unique.tolist(), counts.tolist())))
