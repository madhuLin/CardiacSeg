...
def main():
    ...
    action="store_true",
    default=False,
    help="是否啟用 Test-Time Augmentation（左右 / 前後 flip 等）"
    )
    parser.add_argument(
        "--save_prob",
        action="store_true",
        default=False,
        help="是否存 softmax 機率圖到 infer_dir/prob/*.npy"
    )
    ...
    for exp_id in exp_ids:
        ...
        if base_id not in EXPERIMENTS:
            print(f"[WARN] exp_id={exp_id} (base_id={base_id}) 不在 EXPERIMENTS 裡，跳過。")
            continue

        cfg = EXPERIMENTS[base_id]
        if not cfg.get("enabled", True):
            print(f"[INFO] base_id={base_id} enabled=False，跳過 exp_id={exp_id}。")
            continue

        model_name = cfg["model_name"]
        params: Dict[str, Any] = cfg["params"]
        print(f"\n========== Inference for {exp_id} (base={base_id}, model={model_name}) ==========")
